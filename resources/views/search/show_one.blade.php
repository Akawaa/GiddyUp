    ...
    <div class="container">
        <div class="card card-secondary">
            <h3 class="card-content">
                @for($i=0;$i<count(session()->get('villes'));$i++)
                    @if(session()->get('villes')[$i][0]->trajet_id == $trajet->trajet_id)

                        {{ session()->get('villes')[$i][0]->ville_depart}}

                  <i class="material-icons icon-valign">arrow_forward</i>

                        {{ session()->get('villes')[$i][0]->ville_arrivee }}
                    @endif
                @endfor
                <span class="light">à {{ date('H\hi',strtotime($trajet->trajet_heure)) }}</span>
            </h3>
        </div>
        <div class="row">
            ...
            <div class="col m5 s12">
                <div class="card">
                    <div class="card-content row center-align">
                        <div class="col s6">
                            <h3>@for($i=0;$i<count(session()->get('prix'));$i++)
                                    @if(session()->get('prix')[$i][0]->trajet_id == $trajet->trajet_id)
                                        {{ session()->get('prix')[$i][0]->prix}}
                                    @endif
                                @endfor€</h3>
                            <h4 class="light">par place</h4>
                        </div>
                        <div class="col s6">
                            <h3>@for($i=0;$i<count(session()->get('places'));$i++)
                                    @if(session()->get('places')[$i][0]->trajet_id == $trajet->trajet_id)
                                        {{ $trajet->trajet_place-session()->get('places')[$i][0]->nbPlace}}
                                    @endif
                                @endfor places</h3>
                        ...
                        {{ Form::open(array('url' => '/sign')) }}
                                    ...
                                    @for($i=0;$i<count(session()->get('villes'));$i++)
                                        @if(session()->get('villes')[$i][0]->trajet_id == $trajet->trajet_id)
                                            ...
                                            {{ Form::hidden('depart',session()->get('villes')[$i][0]->insee_depart) }}

                                            {{ Form::hidden('arrivee',session()->get('villes')[$i][0]->insee_arrivee) }}

                                        @endif
                                    @endfor
                        <div class="row">
                                {{ Form::button('Réserver ma place !',['class'=>'waves-effect waves-light btn btn-primary col l11','type'=>'submit']) }}
                        </div>
                         {{ Form::close() }}
                    </div>
                </div>
